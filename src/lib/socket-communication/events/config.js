...
export default {
	...
	DNSServersList( socket, message ) {
		...
		Object.keys( message ).forEach( ( serverAddress ) => {
		...
		currentServerList.forEach( ( server ) => {
	...
	StaticEntriesList( socket, message ) {
		...
		Object.keys( message ).forEach( ( host ) => {
			const hostData = message[ host ];

			Object.keys( hostData ).forEach( ( entryType ) => {
		...
		currentStaticEntriesList.forEach( ( entry ) => {
